...
const componentQuery = computed(() => {
  return route.query.type === "register" ? Register : Login;
})
...
<template>
  <div class="login">
    ...
    <div class="container">
      <div class="content">
        <component :is="componentQuery" />
        <nav role="controls" class="options-wrapper">
          <button @click="changeForm" class="toggle-button">
            {{
              route.query.type === 'register' ? "Already have an account? Log In" : "Create account"
            }}
          </button>
          <button v-if="route.query.type !== 'register'" class="toggle-button" @click.prevent="resetPassword">
            Forgot password?
          </button>
        </nav>
      </div>
